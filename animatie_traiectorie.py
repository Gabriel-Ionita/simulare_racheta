...
y_km = y_vals / 1000
z_km = h_vals / 1000
line, = ax.plot([], [], [], 'r', label="Traiectoria")

# Text pentru altitudine
altitude_text = ax.text2D(0.05, 0.95, "", transform=ax.transAxes)
...
def update(num, x_km, y_km, z_km, line, altitude_text):
    line.set_data(x_km[:num], y_km[:num])
    line.set_3d_properties(z_km[:num])
    altitude_text.set_text(f"Altitudine: {z_km[num]:.2f} km")
    return line, altitude_text

# Configurarea animației
ani = FuncAnimation(fig, update, frames=len(t_vals), fargs=(x_km, y_km, z_km, line, altitude_text), interval=50, blit=True)
